...
public class wc {
	...
	public static String Output(String[] args, Filehandler handler, String buffer) throws IOException {
		...
		if (flaga == 1) {
			System.out.println(handler.fileName + ", " + "代码行/空行/注释行: " + handler.countCodeLine + "/"
					+ handler.countSpaceLine + "/" + handler.countNoteLine);
			buffer += handler.fileName + ", " + "代码行/空行/注释行: " + handler.countCodeLine + "/" + handler.countSpaceLine
					+ "/" + handler.countNoteLine + "\r\n";
		}
		return buffer;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		List<String> listargs = new ArrayList<String>();//统计args中的参数，使用list便于操作
		listargs = Arrays.asList(args);
		String path = null;// 统计的文件的路径
		String stopListPath = null;// 停用表的路径
		String outputPath = null;// 输出文件的路径
